...
% Convert the date column to quaterly
ETS_quarters = datetime(year(ETSprice_month{:,1}), (quarter(ETSprice_month{:,1})-1)*3+1, 1);

% Compute the quaterly mean of ETS price (raw data are monthly)
mean_pe_obs = splitapply(@mean, ETSprice_month{:, 2}, findgroups(ETS_quarters));

% Create a database with quarter, year and quaterly average
